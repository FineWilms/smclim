...
Subroutine getsmdata(climfile,dataout,lsmask,grid,tlld,ccdim,wlev,lonlat,xyz,axyz,bxyz)
...
Real, dimension(1:ccdim(1),1:ccdim(2),1:55+4*wlev), intent(out) :: dataout
...
Allocate(coverout(1:arrsize(1,2),1:arrsize(2,2),1:55+4*wlev))
coverout=0.
...
If (nf_inq_varid(ncid,'urbansm',varid).EQ.nf_noerr) Then
  Write(6,*) "Reading urbansm"
  varname=(/ 'urbansm', 'm3/m3' /)
...
if (nf_inq_varid(ncid,'uic',varid).eq.nf_noerr) then
  ...
  varname=(/ 'uic', 'm/s' /)
  call getmeta(ncid,varname,coverout(1:arrsize(1,2),:,54+4*wlev),arrsize)
  varname=(/ 'vic', 'm/s' /)
  call getmeta(ncid,varname,coverout(1:arrsize(1,2),:,55+4*wlev),arrsize)
end if
...
Do ilon=1,ncsize(1)
  Do ilat=1,ncsize(2)
    ...
    if (coverout(ilon,ilat,1).eq.0.) then
      ...
      coverout(ilon,ilat,47:55+4*wlev)=0.
    end if
...
Do lcj=1,ccdim(2)
  Do lci=1,ccdim(1)
    ...
    if (1-nint(lsmask(lci,lcj)).EQ.1) then
      ...
      countm(lci,lcj)=1
      dataout(lci,lcj,47:55+4*wlev)=0.
      counto(lci,lcj)=1
    end If
  End Do
End Do
  ...
  Do lcj=1,ccdim(2)
    Do lci=1,ccdim(1)
      ...
      if (counto(lci,lcj).eq.0.and.coverout(i,j,47).ne.0.) then
        dataout(lci,lcj,47:55+4*wlev)=coverout(i,j,47:55+4*wlev)
        counto(lci,lcj)=1
      end if
    ...
    Do lcj=1,ccdim(2)
      Do lci=1,ccdim(1)
        If (counto(lci,lcj).EQ.0) then
          call findnear(pxy,lci,lcj,sermask,rlld,ccdim)
	      dataout(lci,lcj,47:55+4*wlev)=dataout(pxy(1),pxy(2),47:55+4*wlev)
